# نقل مجلد «متجر علي جدّي» (أو نسخة قديمة من AliJaddiAccount) إلى: ...\AliJaddi\Store AliJaddi
# المسارات تُشتق من موقع السكربت.
...
$storeRoot = Split-Path $PSScriptRoot -Parent
$alijaddiRoot = Split-Path $storeRoot -Parent
# المصدر الافتراضي: مجلد باسم المتجر على سطح المكتب؛ إن لم يوجد جرّب AliJaddiAccount
$src = Join-Path $desktop "Store AliJaddi"
if (-not (Test-Path -LiteralPath $src)) {
    $src = Join-Path $desktop "AliJaddiAccount"
}
$dest = Join-Path $alijaddiRoot "Store AliJaddi"

if (-not (Test-Path -LiteralPath $src)) {
    Write-Error "المصدر غير موجود. جرّب وضع المجلد على سطح المكتب باسم «Store AliJaddi» أو «AliJaddiAccount»، أو عدّل `$src في السكربت."
    exit 1
}
