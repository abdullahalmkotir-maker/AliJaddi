# -*- coding: utf-8 -*-
"""تثبيت حزمة Ali12 تحت مجلد المستخدم (بدون صلاحيات إدارية عادة)."""
...
def install(source_root: Path | None = None, install_root: Path | None = None) -> Path:
    """
    ينسخ التطبيق إلى LOCALAPPDATA\\Ali12\\app، ينشئ venv، يثبّت المتطلبات،
    ويضع اختصاراً على سطح المكتب (ويندوز) لسطر أوامر Ali12.
    """
    ...
    if not launcher.is_file():
        launcher = app_dir / "run_ali12.py"
    if os.name == "nt" and launcher.is_file():
        lnk = desktop_path() / "Ali12 — أدوات التدريب.lnk"
