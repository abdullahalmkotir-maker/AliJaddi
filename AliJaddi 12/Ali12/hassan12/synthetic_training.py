...
وسد ثغرات وظواهر فقدان/اضطراب بيانات — مناسب لـ JSONL ولتغذية نماذج تعلم.
"""
...
VIRTUAL_PATHS = [
    ...
    r"C:\Users\المستخدم\Desktop\AliJaddi\AliJaddi 12\Ali12\data",
...
SECURITY_MEASURES = [
    ...
    "تثبيت التحديثات من المصدر الرسمي فقط لتقليل البرمجيات الخبيثة",
...
PHENOMENA = [
    ...
    ("رفض الحذف «قيد الاستخدام»", "تطبيق يوفِ الملف", "إغلاق التطبيقات أو إعادة التشغيل"),
...
def _build_explorer_pair(rng: random.Random, idx: int) -> dict[str, Any]:
    ...
    user = rng.choice(USER_QUERY_TEMPLATES).format(
        path=path,
        sym=PHENOMENA[rng.randrange(len(PHENOMENA))][0],
        action=action,
        ctx="مزامنة المجلدات",
        n=idx + 1,
    )
...
def _build_security_pair(rng: random.Random, idx: int) -> dict[str, Any]:
    ...
    assistant = (
        ...
        f"إذا وُجد تعارض مزامنة، اوقف المزامنة المزدوجة على نفس الجذر؛ "
