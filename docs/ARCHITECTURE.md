# معمارية منصّة علي جدّي (Beta 0.5.1)
...
- **المعيار الرسمي (مثل Blender):** `AliJaddi-Beta-0.5.1-Setup.exe` — مثبّت Inno (`AliJaddi_Setup.iss`): Program Files، قائمة ابدأ، تسجيل إزالة في «التطبيقات». يُبنى تلقائياً مع `scripts/build_windows_release.ps1` ويتطلب **Inno Setup 6** على جهاز البناء؛ للتخطي مؤقتاً: `ALIJADDI_SKIP_INNO=1` (ZIP فقط — ليس القناة الرئيسية).
- **مكمّل:** `تنزيل/windows/AliJaddi-Beta-0.5.1-Windows.zip` والمجلد المفكوك — نسخة محمولة دون مثبّت.
