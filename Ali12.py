...
**نطاق حالي (أقوى):** سطح المكتب — متجر المنصّة (معيار ``store_consent_v2``: تثبيت تلقائي في «تطبيقات علي جدي»)، مثبّت المنصّة **Inno**، ZIP، تشغيل hosted، إزالة.
...
def _default_rules() -> list[tuple[str, RuleFn, str]]:
    ...
    hints: list[tuple[str, RuleFn, str]] = [
        ...
        _rule(
            "launch_nonzero_exit",
            sc_launch_exit,
            "انتهى التشغيل برمز خروج غير صفري. راجع السجل في لوحة «تشغيل داخل المنصّة»؛ غالباً نقص حزمة أو خطأ في السكربت — يُدرَّج ذلك لتدريب Ali12 على منصّة علي جدّي.",
        ),
    ]
    return hints
...
def training_payload_stub(
    *,
    ...
    """
    row: dict[str, Any] = {
        "assistant_id": ALI12_ASSISTANT_ID,
        "task": "install_help",
        "event_kind": event_kind,
        "model_id": model_id,
