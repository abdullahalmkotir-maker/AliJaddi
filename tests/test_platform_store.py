...
def test_platform_update_from_registry(monkeypatch):
    monkeypatch.setattr(ps, "ALIJADDI_VERSION", "0.5.2-beta")
    reg = {"platform": "0.5.2-beta", "models": []}
    assert ps.platform_store_update_version(reg) is None
    assert ps.platform_store_update_version({"platform": "0.5.2-beta"}) is None


def test_platform_update_when_remote_newer(monkeypatch):
    monkeypatch.setattr(ps, "ALIJADDI_VERSION", "0.4.1-beta")
    assert ps.platform_store_update_version({"platform": "0.5.2-beta"}) == "0.5.2-beta"
